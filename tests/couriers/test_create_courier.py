...
class TestCreateCourier:
    ...
    def test_create_courier(self):
        ...
        status_code, response_context = courier.create_courier()
    ...
    def test_create_courier_missing_field(self, missing_field):
        courier = CourierMethods
        status_code, response_context = courier.create_courier_missing_field(missing_field)
        assert status_code == 400 and response_context['message'] == CourierErrors.REQUIRED_FIELDS_MISSING, f'Status is {status_code} == 400, context is {response_context}'

    @allure.title('Проверка недоступности создания курьера с тем же логином')
    def test_create_same_courier_error(self):
        courier = CourierMethods
        status_code, response_context = courier.create_same_courier()
    ...
    def test_create_same_courier_error_message(self):
        courier = CourierMethods
        status_code, response_context = courier.create_same_courier()
        assert status_code == 409 and response_context['message'] == CourierErrors.LOGIN_ALREADY_EXISTS, f'Status is {status_code} == 409, context is {response_context}'
